...
| 16  | Verify persistence       | Re-open the shift; tasks, statuses and completions are unchanged                                                                 |
| 17  | Restart the service      | `fly apps restart` / platform restart / `docker restart shiftpilot`                                                              |
| 18  | Verify persistence again | Reload. The shift and tasks survive. **If they vanish, the volume is not mounted where `DATABASE_PATH` points**                  |
...
- Step 18 losing data when you expected a persistent volume. If you deliberately deployed on
  ephemeral storage (Option A in `docs/deployment.md`), this is expected — say so in the
  demo rather than implying persistence.
