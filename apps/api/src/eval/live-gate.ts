...
/**
 * Resolve a live context or exit non-zero. Exit code 2 means "refused to run",
 * distinct from 1 ("ran and failed"), so a CI or wrapper script can tell a
 * missing opt-in apart from a genuine failure.
 */
export function requireLiveContext(env: NodeJS.ProcessEnv): LiveContext {
  ...
  if (env.AI_PROVIDER !== "claude") problems.push("AI_PROVIDER is not set to claude")
